      ...
      <div className="space-y-2">
        ...
        <div>
          ...
          <input
            ...
            placeholder={displayTotal > 0 ? `Min. ₱${displayTotal.toFixed(2)}` : "0.00"}
          ...
          {!cartIsEmpty && amountPaid && !isAmountSufficient && (
            <p className="text-xs text-red-500 mt-0.5">Amount must be at least ₱{displayTotal.toFixed(2)}</p>
          )}
        </div>
        ...
        {amountPaid && isValidAmountPaid && !cartIsEmpty && (
          <div className={`flex justify-between text-sm font-medium mt-1 ${isAmountSufficient ? 'text-blue-600' : 'text-red-600'}`}>
            ...
            <span>{isAmountSufficient ? `₱${change.toFixed(2)}` : `-`}</span>
          </div>
        )}
      </div>
